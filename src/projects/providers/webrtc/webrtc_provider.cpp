...
namespace pvd
{
	...
	bool WebRTCProvider::Start()
	{
		...
		// Initialize WhipServer
		_whip_server = std::make_shared<WhipServer>(webrtc_bind_config);
		_whip_server->Start(WhipObserver::GetSharedPtr(), has_port ? &signalling_address : nullptr, has_tls_port ? &signalling_tls_address : nullptr, worker_count);
	...
	bool WebRTCProvider::OnDeleteHost(const info::Host &host_info)
	{
		...
		if (_signalling_server != nullptr)
		{
			_signalling_server->RemoveCertificate(host_info.GetCertificate());
		}

		if (_whip_server != nullptr)
		{
			_whip_server->RemoveCertificate(host_info.GetCertificate());
		}
